...
def replace_leaf(t, old, new):
    ...
    if (is_leaf(t)):
        ...
        currentBranches = branches(t)
        for i in range(0, len(currentBranches)):
            currentBranches[i] = replace_leaf(currentBranches[i], old, new)
        return tree(label(t), currentBranches)
...
def prune_leaves(t, vals):
    ...
    if (is_leaf(t)):
        ...
        currentBranches = branches(t)
        for i in range(0, len(currentBranches)):
            currentBranches[i] = prune_leaves(currentBranches[i], vals)
        return tree(label(t), [bran for bran in currentBranches if bran!=None ])
# Mobiles
...
def balanced(m):
    ...
    if (balanced(left_end) and balanced(right_end)):
        left_torque = length(left_side) * total_weight(left_end)
        right_torque = length(right_side) * total_weight(right_end)
...
def two(f):
    ...
    "*** YOUR CODE HERE ***"
    return lambda f: lambda x: f(f(zero(f)(x)))

three = successor(two)
#three = lambda f: lambda x: f(f(f(zero(f)(x))))
def church_to_int(n):
